...
1、确定状态，最后一步如何花费最小呢，之前花费最小额度 + 当前剩余可选的最小额度。如何确定上一个颜色呢？
   当最后一种颜色为红色时，花费最小值为 dp[i][0] = min(dp[i - 1][1], dp[i - 1][2]) + costs[i-1][0]
   当最后一种颜色为蓝色时，花费最小值为 dp[i][1] = min(dp[i - 1][0], dp[i - 1][2]) + costs[i-1][1]
   当最后一种颜色为绿色时，花费最小值为 dp[i][2] = min(dp[i - 1][0], dp[i - 1][1]) + costs[i-1][2]
...
func main() {
	...
	for i := 1; i < length; i++ {
		dp[i][0] = min2(dp[i-1][1], dp[i-1][2]) + costs[i-1][0]
		dp[i][1] = min2(dp[i-1][0], dp[i-1][2]) + costs[i-1][1]
		dp[i][2] = min2(dp[i-1][0], dp[i-1][1]) + costs[i-1][2]
	}
